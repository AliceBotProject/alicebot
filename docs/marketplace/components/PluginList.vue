...
const props = await (
  await fetch(
    "https://raw.githubusercontent.com/MarleneJiang/issue-ops/master/plugins.json"
  )
).json();
const pageSize = 10;
  ...
  <div>
    <div class="tab">
      ...
      <div class="select-list">
        <button
          class="select-button"
          @click="setType('plugin')"
          :class="{ active: type == 'plugin' }"
        >
          插件
        </button>
        <button
          class="select-button"
          @click="setType('adapter')"
          :class="{ active: type == 'adapter' }"
        >
          适配器
        </button>
        <button
          class="select-button"
          @click="setType('example')"
          :class="{ active: type == 'example' }"
        >
          机器人
        </button>
      </div>
      ...
      <div class="card" v-for="(item, index) in pluginLists" :key="index">
        <div class="card-top">
          <div class="card-head">
            ...
            <div class="card-github" @click="openGithub(item.url)">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="21"
                height="21"
